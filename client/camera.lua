...
-- Events
RegisterNetEvent('sasp:client:ActiveCamera', function(cameraId)
...
RegisterNetEvent('sasp:client:DisableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('sasp:client:EnableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('sasp:client:SetCamera', function(key, isOnline)
    ...
    elseif type(key) == 'number' then
        ...
        error('sasp:client:SetCamera did not receive the right type of key\nreceived type: ' .. type(key) .. '\nreceived value: ' .. key)
    end
end)
