...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            local veh = NetToVeh(netId)
            SetCarItemsInfo()
            SetVehicleNumberPlateText(veh, Lang:t('info.sasp_plate')..tostring(math.random(1000, 9999)))
            SetEntityHeading(veh, coords.w)
...
RegisterNetEvent('qb-saspjob:ToggleDuty', function()
    ...
    TriggerServerEvent("sasp:server:UpdateBlips")
end)
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['qb-target']:AddBoxZone("saspDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "saspDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
...
CreateThread(function()
    ...
    evidenceCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            if PlayerJob.name == "sasp" and onDuty then
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("stash_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "stash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-saspjob:client:target:OpenStash",
                        icon = "fas fa-sign-in-alt",
    ...
    for k, v in pairs(Config.Locations["armory"]) do
        exports['qb-target']:AddBoxZone("Armory_"..k, vector3(v.x, v.y, v.z), 2, 1, {
            name = "Armory_"..k,
            heading = 11,
            debugPoly = false,
            minZ = v.z - 1,
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        exports['qb-target']:AddBoxZone("Helicopter_"..k, vector3(v.x, v.y, v.z), 2, 2, {
            name = "Helicopter_"..k,
            heading = 11,
            debugPoly = false,
            minZ = v.z - 1,
            ...
            options = {
                {
                    type = "client",
                    event = "qb-sasp:client:spawnHelicopter",
                    icon = "fas fa-sign-in-alt",
                    label = "Take out Air-One",
                    job = "sasp",
                },
            },
            distance = 1.5
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            inGarage = true
            if onDuty and PlayerJob.name == 'sasp' then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'sasp:client:VehicleMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
...
CreateThread(function ()
    ...
    while true do
        ...
        if inStash and PlayerJob.name == "sasp" then
            ...
            if IsControlJustReleased(0, 38) then
                TriggerServerEvent("inventory:server:OpenInventory", "stash", "saspstash_"..QBCore.Functions.GetPlayerData().citizenid)
                TriggerEvent("inventory:client:SetCurrentStash", "saspstash_"..QBCore.Functions.GetPlayerData().citizenid)
            end
        else
            sleep = 1000
        end
...
CreateThread(function ()
    ...
    while true do
        ...
        if inTrash and PlayerJob.name == "sasp" then
            if onDuty then sleep = 5 end
            if IsControlJustReleased(0, 38) then
                TriggerServerEvent("inventory:server:OpenInventory", "stash", "sasptrash", {
                    maxweight = 4000000,
                    slots = 300,
                })
                TriggerEvent("inventory:client:SetCurrentStash", "sasptrash")
            end
        else
            sleep = 1000
        end
...
CreateThread(function ()
    ...
    while true do
        local sleep = 1000
        if inAmoury and PlayerJob.name == "sasp" then
            if onDuty then sleep = 5 end
            if IsControlJustReleased(0, 38) then
