...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "sasp" and PlayerJob.name ~= "sasp" then
...
RegisterNetEvent('sasp:client:sendBillingMail', function(amount)
...
RegisterNetEvent('sasp:client:UpdateBlips', function(players)
    if PlayerJob and (PlayerJob.name == 'sasp' or PlayerJob.name == 'ambulance') and
        onDuty then
        if DutyBlips then
            for _, v in pairs(DutyBlips) do
                RemoveBlip(v)
...
RegisterNetEvent('sasp:client:saspAlert', function(coords, text)
...
RegisterNetEvent('sasp:client:SendToJail', function(time)
    TriggerServerEvent("sasp:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('sasp:client:SendsaspEmergencyAlert', function()
    local Player = QBCore.Functions.GetPlayerData()
    TriggerServerEvent('sasp:server:saspAlert', Lang:t('info.officer_down', {lastname = Player.charinfo.lastname, callsign = Player.metadata.callsign}))
