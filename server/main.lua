...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.name == "sasp" or v.PlayerData.job.name == "ambulance" or v.PlayerData.job.name == "police" or v.PlayerData.job.name == "sheriff") and v.PlayerData.job.onduty then
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {{name = "id", help = Lang:t('info.player_id')}, {name = "time", help = Lang:t('info.jail_time')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        ...
        if time > 0 then
            TriggerClientEvent("sasp:client:JailCommand", src, playerId, time)
        else
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "sasp") and Player.PlayerData.job.onduty) and OtherPlayer then
...
-- Callbacks
QBCore.Functions.CreateCallback('sasp:server:isPlayerDead', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('sasp:GetPlayerStatus', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('sasp:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "sasp" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('sasp:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('sasp:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('sasp:GetCops', function(source, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "sasp" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('sasp:server:IssaspForcePresent', function(source, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "sasp" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        CreateThread(function()
            MySQL.Async.execute("DELETE FROM stashitems WHERE stash='sasptrash'")
        end)
    end
end)
...
RegisterNetEvent('sasp:server:TakeOutImpound', function(plate)
    local src = source
...
RegisterNetEvent('sasp:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.PlayerData.job.name == "sasp" then
...
RegisterNetEvent('sasp:server:EscortPlayer', function(playerId)
    ...
    if EscortPlayer then
        if (Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "sheriff") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
